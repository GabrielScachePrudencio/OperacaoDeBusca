...
void buscadorDeNumeroPelosIndices(int *vetor[tam], int vetorInd[tamInd], int numProc){
    ...
    for(int i = 0; i < tamInd; i++){ // ele vai ver pegar cada indice que e um numero 100 200
        if(numProc >= vetorInd[i]){ // e vai pegar o q for o ultimo menor 
            ...
            for(int j = comeco; j < comeco + (tam/tamInd) && achou != 1 ; j++){ //usando esse inicio define o maximo que vai que ate o fim do indice
                if(vetor[j] == numProc){
                    achou = 1;
                    printf("\n\nO indice do numero %d e %d dentro do indice %d ou %d", numProc, j, i, vetorInd[i]);
                    j = comeco + (tam/tamInd);
...
- usar busca binaria no buscadorDeNumeroPelosIndices
*/
